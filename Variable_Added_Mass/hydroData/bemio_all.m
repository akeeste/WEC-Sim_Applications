...
vars1D = {'fk_re', 'fk_im', 'fk_ma', 'fk_ph', ...
...
for i = nAngles0+1 : length(angles)
    ...
    for iVar = 1:length(vars1D)
        allHydro(i).(vars1D{iVar}) = allHydro(ind1).(vars1D{iVar}) * (1-dAngle) + ...
                                     allHydro(ind2).(vars1D{iVar}) * dAngle;
    end
    for iVar = 1:length(vars2D)
        allHydro(i).(vars2D{iVar}) = allHydro(ind1).(vars2D{iVar}) * (1-dAngle) + ...
                                     allHydro(ind2).(vars2D{iVar}) * dAngle;
    end
end
